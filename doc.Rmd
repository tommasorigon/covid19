...
Si assume che la serie storica $Y_t$ (ovvero i nuovi casi, decessi giornalieri e ricoveri in terapia intensiva) sia il frutto del prodotto di tre componenti: un livello (trend), una stagionalità e una componente di "rumore". In formule, scriveremo
$$
...
$$
  b_{t+1} = b_t + z_t,
$$
dove $z_t$ è una quantità aleatoria a media zero e varianza $\sigma_z^2$. Si suppone inoltre che anche il fattore stagionale (settimanale) e il rumore evolgano nel tempo in modo casuale. Il primo nel modo seguente
$$
...
La stima delle tre varianze avviene col metodo della massima verosimiglianza supponendo che le tre quantità aleatorie siano variabili aleatorie normali (e quindi $Y_t$ sia un processo log-normale). Le quantità non osservabili $l_t$ e $b_t$ sono invece estratte per mezzo di procedure che minimizzano l'errore quadratico medio (*Kalman filter* e *Kalman smoother*). Questi algoritmi ci restituiscono le quantità
$$
...
$$
da cui stimiamo il **tasso di crescita netto** riportato nei grafici
$$
